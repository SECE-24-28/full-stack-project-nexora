          ...
          {data.questions.map((q: Question) => (
            ...
            >
              <div className="flex justify-between items-start mb-4 gap-4">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${q.difficulty.toUpperCase() === "EASY"
              ...
              <div className="bg-slate-50 p-3 rounded-xl mb-6 border border-slate-100 flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] font-extrabold px-2.5 py-1 rounded-md uppercase tracking-wider ${(q.userStatus || "NOT_STARTED") === "SOLVED"
                      ? "bg-emerald-100 text-emerald-700"
                      : (q.userStatus || "NOT_STARTED") === "IN_PROGRESS"
                ...
                <div className="mb-3">
                  <div className="flex flex-wrap gap-2">
                    {q.categories.map((cat, idx) => (
                      <span
                        key={idx}
                        className="bg-indigo-50 text-indigo-700 text-[10px] px-2 py-1 rounded-md font-bold uppercase"
                      >
                        {cat.name}
                      </span>
                    ))}
                ...
                <div>
                  <div className="flex flex-wrap gap-2">
                    {q.companies.map((comp, idx) => (
                      <span
                        key={idx}
                        className="bg-slate-100 text-slate-600 text-[10px] px-2 py-1 rounded-md font-bold uppercase border border-slate-200"
                      >
                        {comp.name}
                      </span>
                    ))}
